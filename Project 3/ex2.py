...
def gini_index(groups, classes):
	...
	for group in groups:
		...
		prior = [float((group[:, -1] == i).sum()) / size for i in range(classes)]
		gini += (size / n_instances) * (1 - sum([prior[i]*prior[i] for i in range(classes)]))
        ####################################################################
...
def split(node, max_depth, min_size, depth):
	...
	if left.size == 0 or right.size == 0:
		node['left'] = node['right'] = leaf(left) if right.size == 0 else leaf(right)
		return
	# check for max depth
	if depth >= max_depth:
